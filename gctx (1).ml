...
type thickness = {thicknessValue:int}

let thick    : thickness = {thicknessValue = 7}
let thin     : thickness = {thicknessValue = 1}
(*******************************)
...
let draw_points (g: gctx) (pl: position list) : unit = 
  set_graphics_state g;
  List.iter (fun m -> draw_point g m) pl
   
  (*works too*)
  (*List.iter (fun a acc -> 
  let (x,y) = ocaml_coords g a in Graphics.plot x y) pl *)
