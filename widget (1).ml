...
let list_layout (pair: widget -> widget -> widget)
  ...
  List.fold_right (fun x y -> pair x y) ws (space (0,0))
...
let make_controller (v: 'a) : 'a value_controller = 
  let currValue = {contents = v } in
  let listeners = {contents = []} in (*list of functions*)
  {
    add_change_listener = 
    (fun x -> listeners.contents <- [x] @ listeners.contents );
    get_value = (fun () -> currValue.contents);
    change_value = fun x -> currValue.contents <- x ; 
    List.iter (fun h -> h currValue.contents) listeners.contents
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  (*failwith "idk"  *)
  (* A function to display the check box  *)
  let (w, lc) = label s in

  (** Construct a label widget and its controller. *)
  let boolControl = make_controller init in 
  let checkWidget = {
    repaint = (
          fun (g: Gctx.gctx) ->

          let translateG =  
          Gctx.translate g (0, 0) in Gctx.draw_rect translateG (0,0) (15,15); 

            if boolControl.get_value() 
            then begin 
            ( let translateG =  
            Gctx.translate g (0, 0) in Gctx.draw_string translateG (5,-5) "x"); 
            end
            else ();
          
          
         let translateG =  Gctx.translate g (20, 0) in 
         w.repaint translateG
         )
         ;
    (*should work handle*)
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> 
    ...
    then boolControl.change_value ( not(boolControl.get_value()) ) 
    )

    ; 

    size = (fun () -> (120, 20))
  } in (border  (checkWidget), boolControl)


(* size: 
size of the label + size of the box 
repaint: 
draw the label and the box(with or without the check)
1. make a label widget and call the function
2. call draw string directly 
Draw the string and the draw the box
(have to start it to the right 
of the string or draw at (0,0) and then offset it)
graphics context let you know where to do it relative to where the canvas is 
Decide whether to draw the box or not -> holds a bool 
inital value is init, store it at 
someplace where it will be updated(value controller)
(controller only need to hold on to 1 bool)
Paint the outside of the box, if true, paint the true(if statement in repaint) 
check box will be more like label/button widget 

Handler(called whenever an event happens) 
notice there is a mosue down, changes the value 
*)

(* ----sliders------
when slider is procesing mouse drag, what is being changed 
*)

let sliders (init: int) (s: string) : widget * int value_controller =
    (** Construct a label widget and its controller. *)
  (* A function to display the slider box  *)
  let (w, lc) = label s in
  let intControl = make_controller init in 
  let sliderWidget = {
    repaint = (
          fun (g: Gctx.gctx) ->
          (*calculates the translation needed for the box*)
          let translateG =  Gctx.translate g (0, 0) in 
            (*obtains the length of the slider from the value controller*)
            let length =  intControl.get_value() in
              (*rescale the value to draw the box*)
              let rescaledLength = length * 5 + 3 in 
              (*account for the edge case when length is <=0 *)
              if length <= 0 
                then Gctx.draw_rect translateG (0,0) (1,20)
                (*regular case, draw the slider rectangle*)
                else let g_new = (Gctx.with_color g Gctx.black) in
                Gctx.fill_rect g_new (0, 0) (rescaledLength, 20); 
            (*draws the widget*)      
            let translateG =  Gctx.translate g (160, 0) in 
            w.repaint translateG
    ...
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> 
      ...
      then let (mouse_x, mouse_y) = Gctx.event_pos e g in 
      (*gets the x position of mouse and set that as length*)
      let length =  mouse_x in
        (*rescale the length*)
        let scaledLength = length / 5 in 
          if scaledLength <= 0 
          (*account for edge case of when scaledLength is <=0 *)
          then intControl.change_value 1
          else intControl.change_value scaledLength
    )

    ; 

    size = (fun () -> (150, 20))
  } in (border  (sliderWidget), intControl)
